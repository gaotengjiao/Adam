 ...
 * 接口成功信息返回
 * */
function SONEOK($Result){
	$returnmess['resultnum'] = "0";
	$returnmess['result_mess'] = "成功";
	$returnmess['result'] = $Result;
	return $returnmess;exit;
}
/*
 * 账单错误
 * */
function SONEBILLERROR(){
	$result['resultnum'] = "133";
	$result['result_mess'] = "账单错误";
	$result['result'] = "";
	return $result;
	exit;
}
 ...
 * */
function DONTSTAGE(){
	$result['resultnum'] = "132";
	$result['result_mess'] = "当前用户不允许进行分期操作";
	$result['result'] = "";
	return $result;
	exit;
}
...
/*
     * 添加用户标签
     * */
function AddUserLabel($Uid,$KeyWord){
	$MemberTable = M("member");
	$UserOldLabel = $MemberTable->where("uid = $Uid")->field("label")->find();
	$label = $UserOldLabel['label'];
	$data['label'] = $label.",".$KeyWord;
	if($MemberTable->where("uid = $Uid")->save($data)){
		return SONEOK();exit;
	}else{
...
function TIMESHOW($date_time){
	...
	if($cha<60){
		...
		$minute = floor($cha/60);
		return $minute."分钟前";exit;
	}elseif($cha>3600 && $cha<86400){
		$hour = floor($cha/3600);
		return $hour."小时前";exit;
	}elseif($cha>86400 && $cha< 604800){
		$day  = floor($cha/86400);
		return $day."天前";exit;
	}elseif ($cha>604800){
		return date("Y-m-d", $time);exit;
	}
}
