...
class ShortUrl_pub extends Wxpay_client_pub
{
	...
	{		
		try 
		{
			...
			{
				throw new SDKRuntimeException("long_url！"."<br>");
			}
		   	$this->parameters["appid"] = WxPayConf_pub::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = WxPayConf_pub::MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
...
class JsApi_pub extends Common_util_pub
{
	...
	var $curl_timeout;//curl超时时间
	var $nickname;//curl超时时间
	var $headimgurl;//curl超时时间
	var $art;//curl超时时间
...
*/	
	...
	function getOpenid()
	{
		...
		$json = file_get_contents("https://api.weixin.qq.com/sns/oauth2/access_token?appid=".$appid."&secret=".$secret."&code=".$this->code."&grant_type=authorization_code");
	...
	function getNickname()
	{
		...
		$this->nickname = $this->art['nickname'];
...
function getHeadimgurl()
	{
		...
		$this->headimgurl = $this->art['headimgurl'];
