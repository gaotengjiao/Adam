...
*/
	...
	$linkID=@mysql_connect("rm-2ze921c76782e3772o.mysql.rds.aliyuncs.com","qiaohailin","qiao123@");
		...
		$stateu = $_GET['state'];
		list($corwdid, $userid)= explode('=',$stateu);
		...
		$message = mysql_fetch_array(mysql_query("select * from weixin.wechat_message where openid='".$openid."'"));	//查询username
		
		$member = mysql_fetch_array(mysql_query("select * from phome_enewsmember where userid='".$userid."'"));	//查询username
		
		$newstime = date('Y-m-d H:i:s');
		
		$count = mysql_fetch_array(mysql_query("select count(*) from weixin.wechat_message where openid='".$openid."'"));	//查询username
		//if($count[0] > 0){
		//	$update = mysql_query("update weixin.wechat_message set uname='".$member['username']."' where openid='".$openid."'");

		//}else{
		//	$insert = mysql_query("insert into weixin.wechat_message(openid,uname,createtime) values('".$openid."','".$member['username']."','".$newstime."')");		
		...
		//if($fasercount[0]>0){
		//	echo "<script>window.location.href='http://b.autostep.com.cn/qiaohailin/H5_zc/index.php/Home/Raiselist/zc_yesok';</script>";
		//}else{
		//}
...
$weixin = new jssdk($url='http://b.autostep.com.cn/qiaohailin/wx_payment/demo/js_api_call.php?code='.$code.'&state='.$corwdid);//借用传入0或1,url是地址默认不借用,当前地址
$wx = $weixin->get_sign();
				...
				function(res){
					...
					if(Cts.indexOf("ok")>0 || Cts.indexOf("OK")>0){
						$(function(){
							$.ajax({
								type: 'POST',
								url: "http://b.autostep.com.cn/qiaohailin/wx_payment/demo/crowdorders_bd.php",
								data:{"ordersmark":ordersmark,"corwdid":corwdid,"userid":userid,"moneyub":moneyub},
								dataType: "text",
								success: function (count) {
									if(count == "0"){
										alert('支付成功');
										window.location.href='http://b.autostep.com.cn/qiaohailin/function/zcok/?from=singlemessage&isappinstalled=0';
									}else if(count== "2"){
										alert("您以众筹过！");
									}else{
										alert('订单生成失败！');
...
<div class="gridContainer clearfix">
   ...
   */
     var cfg = {
		...
		title : '优一步课程可以众筹了！',
		desc : '只需10元，听名师课程',
		image_url : 'http://b.autostep.com.cn/qiaohailin/H5_zc/H5raise/Home/Public/images/home/title.png',
	};
  ...
  wx.config({
	...
	wx.ready(function(){
		wx.onMenuShareTimeline({
			...
			link: "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx4484942683d3f92c&redirect_uri=http://b.autostep.com.cn/qiaohailin/H5_zc/index.php/Hmoe/Index/index&response_type=code&scope=snsapi_userinfo&state=state#wechat_redirect", // 分享链接
