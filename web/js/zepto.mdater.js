(function($){
	...
	$.fn.mdater = function(config){
		if(localStorage.getItem('sNum')&&localStorage.getItem('sNum')!=0){
            //alert(localStorage.getItem('sNum'));
			var defaults = {
				maxDate : null,
				...
				afterShowPanel : function (mask,panel) {
					...
					panel.delegates({
						...
						'.md_ok' : function(){
							...
							var datavalue1=_this.value.year + '-' + monthValue + '-' + dateValue;
							localStorage.setItem('datavalue1',datavalue1);
							_this.input.val(_this.value.year + '-' + monthValue + '-' + dateValue);
							datavalue=_this.value.year + '-' + monthValue + '-' + dateValue;
							_this._hidePanel();
						},
						'.md_cancel' : function(){
							_this._hidePanel();
